...
bool PathCheck(std::vector<int> copy_solution, vector<vector<int>> PathPass)
{
	for (auto a : PathPass)
	{
		auto first = find(copy_solution.begin(), copy_solution.end(), a[0]);
		auto second = find(copy_solution.begin(), copy_solution.end(), a[1]);
		if (first == copy_solution.end() || second == copy_solution.end())
			return false;
		if (first + 1 != second && second + 1 != first)
			return false;
	}
	return true;
}
...
Graph::Graph(int V, int initial_vertex, int end_vertex, std::vector<std::vector<int>> PathPass, vector<int> PointSelect, std::map<std::pair<int, int>, int> PairCost) // constructor of Graph
{
	...
	this->V = V - 1; // assigns the number of vertices
	...
	this->map_edges = PairCost;
	this->PointSelect = PointSelect;
	this->PointSelect.push_back(initial_vertex);
	this->PointSelect.push_back(end_vertex);
	sort(this->PointSelect.begin(), this->PointSelect.end());
}
...
Genetic::Genetic(Graph* graph, int size_population, int generations, int mutation_rate)
{
...
{
	int total_cost = 0;
	solution.push_back(graph->end_vertex);
	// checks if not contains elements repeated
	if (solution.size() != ((unsigned)graph->V) + 1)
		return -1;

	// checks if connections are valid
	if (!PathCheck(solution, graph->PathPass))
		return -1;
...
void Genetic::initialPopulation() // generates the initial population
{
	...
	for (int i = 0; i < graph->V; i++)
	{
		if (graph->PointSelect[i] != graph->initial_vertex)
			parent.push_back(graph->PointSelect[i]);
	}

	for (auto a : graph->PathPass)
	{
		auto first = find(parent.begin(), parent.end(), a[0]);
		auto second = find(parent.begin(), parent.end(), a[1]);
		if (first + 1 != second && first - 1 != second)
		{
			int random = rand() % 2;
			if (random == 0)
			{
				if (first == parent.begin() + 1)
				{
					int temp = *(first + 1);
					*(first + 1) = a[1];
					*second = temp;
				}
				else if (first == parent.end() - 1)
				{
					int temp = *(first - 1);
					*(first - 1) = a[1];
					*second = temp;
			...
			{
				if (second == parent.begin() + 1)
				{
					int temp = *(second + 1);
					*(second + 1) = a[0];
					*first = temp;
				}
				else if (second == parent.end() - 1)
				{
					int temp = *(second - 1);
					*(second - 1) = a[0];
					*first = temp;
...
void Genetic::crossOver(vector<int>& parent1, vector<int>& parent2)
{
	...
	//confirm the pass constraint
	for (auto a : graph->PathPass)
	{
		auto first = find(child1.begin(), child1.end(), a[0]);
		auto second = find(child1.begin(), child1.end(), a[1]);
		if (first + 1 != second && first - 1 != second)
		{
			int random = rand() % 2;
			if (random == 0)
			{
				if (first == child1.begin() + 1)
				{
					int temp = *(first + 1);
					*(first + 1) = a[1];
					*second = temp;
				}
				else if (first == child1.end() - 1)
				{
					int temp = *(first - 1);
					*(first - 1) = a[1];
					*second = temp;
			...
			{
				if (second == child1.begin() + 1)
				{
					int temp = *(second + 1);
					*(second + 1) = a[0];
					*first = temp;
				}
				else if (second == child1.end() - 1)
				{
					int temp = *(second - 1);
					*(second - 1) = a[0];
					*first = temp;
	...
	for (auto a : graph->PathPass)
	{
		auto first = find(child2.begin(), child2.end(), a[0]);
		auto second = find(child2.begin(), child2.end(), a[1]);
		if (first + 1 != second && first - 1 != second)
		{
			int random = rand() % 2;
			if (random == 0)
			{
				if (first == child2.begin() + 1)
				{
					int temp = *(first + 1);
					*(first + 1) = a[1];
					*second = temp;
				}
				else if (first == child2.end() - 1)
				{
					int temp = *(first - 1);
					*(first - 1) = a[1];
					*second = temp;
			...
			{
				if (second == child2.begin() + 1)
				{
					int temp = *(second + 1);
					*(second + 1) = a[0];
					*first = temp;
				}
				else if (second == child2.end() - 1)
				{
					int temp = *(second - 1);
					*(second - 1) = a[0];
					*first = temp;
...
void Genetic::run(std::pair<std::vector<int>, int> &best_solution)
{
	...
	for (int i = 0; i < generations; i++)
	{
		...
		if (best_solution.second >= population[0].second)
		{
			best_solution = population[0];
		}
	}
